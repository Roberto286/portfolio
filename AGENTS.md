...
2. **Dev Server:** Run `mise run example-site`.
   - This will install all dependencies (root & site) automatically.
...
- **`data/`**: The source of truth for the site's layout configuration. Example data can be found under `site/data`.
...
4. Define the data schema in `site/data/<language code>/sections/<section-name>.yaml`.
5. Wrap the rendering in a conditional check (e.g., `if .Site.Params.features.newSection.enable`).

### How to Fix a Bug

1. Reproduce it in `site`.
